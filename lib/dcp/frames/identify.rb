class Dcp
  ...
  IdentifyResponse = Struct.new(:name, :mac_address, :ip_address, :subnet_mask, :description) do
    ...
    def self.parse(frame)
      ...
      i = IDENTIFY_RESPONSE_START
      while i < frame.size do
        block_size = frame[i+2..i+3].unpack('n').first
        case frame[i..i+1]
          when "#{IDENTIFY_BLOCK_OPTION[:device]}#{IDENTIFY_DEVICE_BLOCK[:vendor]}"   # device/vendor
            device.description = frame[i+6..i+6+block_size-2-1]
          when "#{IDENTIFY_BLOCK_OPTION[:device]}#{IDENTIFY_DEVICE_BLOCK[:name]}"     # device/name_of_station
            device.name = frame[i+6..i+6+block_size-2-1]
          when "#{IDENTIFY_BLOCK_OPTION[:ip]}#{IDENTIFY_IP_BLOCK[:ip]}"               # ip/ip
